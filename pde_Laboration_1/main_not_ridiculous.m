...
for m = 1:M
    for n1 = -N:N
         for n2 = -N:N
             ...
             A(m, (n1 + N)*(2*N + 1) + (n2+N) + 1) = exp(1j*kappa*(n1*xp_matrix(m,1) + n2*xp_matrix(m,2)));
...
figure(1)
% test it
mesh(x1_omega, x2_omega, real(v1_approx_matrix))

% compare with expected
figure(2)
v1 = @(x,y) y;
mesh(x1_omega,x2_omega,real(v1(xx1, xx2)))
